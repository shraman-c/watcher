...
If you want to run Watcher from source on Linux, install Python and pip first, then launch the GUI. On first run, `gui.py` will install the missing runtime Python packages it needs automatically.
...
Then clone the repository and start the app:
...
- **pip** package manager
- **Linux desktop libraries** required by your distro for Qt apps if they are not already installed
...
If you skip this step, the GUI will try to install the missing runtime Python packages automatically the first time you run [gui.py](gui.py).
...
---
### Bugs
- [ ] There is a Problem with newly created/downloaded files in the folder it is watching. The Download is failing as the application is trying to move the file while it is being downloaded.
---
### What's next?

- Better and Improved UI/UX
- Enhanced Performance
- Fixing the Downloading issue
- Making native executables for Linux via Flatpak or AppImage
...
*© 2026 Shraman Chaudhuri. All rights reserved.*
